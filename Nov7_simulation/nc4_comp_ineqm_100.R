...
mcmc_sim <- function(db_comb, db_long, burn_in_time, rep_time, B){
  ...
  Y_trt_avg_T=mean(Ytrt_comp[,T_length] - Ytrt_comp[,1])
  Y_crtl_avg_T=mean(Ycrtl_comp[,T_length] - Ycrtl_comp[,1])
  ...
  for (k in 1:rep_time){
    ...
    vtrt[k]<- var(t[,5] - t[,1])/nrow(t)* (1-pi_hat_trt)^2+var(comp[,5] - comp[,1])/nrow(comp)*pi_hat_trt^2
    ...
    vcon[k]<- var(t[,5] - t[,1])/nrow(t)* (1-pi_hat_crtl)^2+var(comp[,5] - comp[,1])/nrow(comp)*pi_hat_crtl^2
    ...
    #estimate u
    Y_mistrt_avg_T=mean(Y_mistrt_imputed[,T_length] - Y_mistrt_imputed[,1])
    Y_miscrtl_avg_T=mean(Y_miscrtl_imputed[,T_length] - Y_miscrtl_imputed[,1])
    ...
    u_trt=theta_comp_trt[T_length] - theta_comp_trt[1]
    u_crtl=theta_comp_crtl[T_length] - theta_comp_crtl[1]
    u_diff = u_trt-u_crtl
...
mle_finder <- function(db_comb, db_long, method = "naive"){
  ...
  if(method == "naive"){
    mu_ctl <- mean(db_comb_ctl[,p] - db_comb_ctl[,1])
    mu_trt <- mean(db_comb_trt[,p] - db_comb_trt[,1])*obs_pi[2] + mu_ctl*(1 - obs_pi[2])
...
## Data summary ----
library(e1071)
data_summary <- function(n, mcmc_data, var_rubin, var_bayes, true_mean){
  mean_mcmc <- c(0)
  var_mcmc_fs <- c(0)
  skew_mcmc <- c(0)
  ...
  for(s in 1:nsim){
    ...
    count_power_bayes[s] <- ifelse(abs(mean_mcmc[s] - true_mean) > sqrt(var_bayes[s])*qt(0.975, n-1), 1, 0)
  }
...
res <- data_summary(nn[1], tmp1$mean_ctl_mcmc,tmp1$var_rubin_est[1,], tmp1$var_bayes_est[1,], 3)
