...
Sigma_posterior<-function(Y_matrix,Y_long,T_length, theta, v0, S0){
  ...
  # S0 it the INVERSE of the second para of Inv-Wishart that negerates Y
  ...
  #second_para <- ginv(S0+S_theta)
  second_para <- S0+S_theta
...
mcmc_sim <- function(db_comb, db_long, burn_in_time, rep_time, B){
  ...
  if(T_length==5){
    name=c('y1','y2','y3','y4','y5')
  }
  if(T_length==10){
    name=c('y1','y2','y3','y4','y5','y6','y7','y8','y9','y10')
  }
  if(T_length==20){
    name=c('y1','y2','y3','y4','y5','y6','y7','y8','y9','y10','y11','y12','y13','y14','y15','y16','y17','y18','y19','y20')
  }
  
  U <- matrix(0, 3, rep_time)
  U_bayes <- matrix(0, 3, rep_time)
  # print("burn-in")
  ...
  Sigma_comp_crtl <- Sigma_crtl
  Sigma_comp_trt <- Sigma_trt
  for (k in 1:rep_time){
    #get the estimate of theta and Sigma
    theta_trt<-get_theta(db_comb_trt, db_long_trt,mu,Sigma_trt,Omega)
    theta_crtl<-get_theta(db_comb_crtl, db_long_crtl,mu,Sigma_crtl,Omega)
    ...
    if(nrow(Ymis)!=0){
      ...
      vtrt[k]<- var(t[,T_length])/nrow(t)* (1-pi_hat_trt)^2+var(comp[,T_length])/nrow(comp)*pi_hat_trt^2
      ...
      comp<- get_complete(db_comb_trt, db_long_trt)$complete
      vtrt[k]<- var(comp[,T_length])/nrow(comp)*pi_hat_trt^2
      Ytrt_complete=rbind(get_complete(db_comb_trt, db_long_trt)$complete,Ymis) 
    }
    ...
    if(nrow(Ymis)!=0){
      ...
      vcon[k]<- var(t[,T_length])/nrow(t)* (1-pi_hat_crtl)^2+var(comp[,T_length])/nrow(comp)*pi_hat_crtl^2
      ...
      comp<- get_complete(db_comb_crtl, db_long_crtl)$complete
      vcon[k]<-var(comp[,T_length])/nrow(comp)*pi_hat_crtl^2
      Ycrtl_complete=rbind(get_complete(db_comb_crtl, db_long_crtl)$complete,Ymis) 
    }
    ...
    # using bayesian model to estimate
    #get the estimate of theta and Sigma
    theta_comp_trt<-get_theta(Ytrt_complete, db_long_trt, mu, Sigma_comp_trt, Omega)
    theta_comp_crtl<-get_theta(Ycrtl_complete, db_long_crtl, mu, Sigma_comp_crtl, Omega)
    
    Sigma_comp_trt<-Sigma_posterior(Ytrt_complete, db_long_trt, T_length, theta_comp_trt, v0, S0)
    Sigma_comp_crtl<-Sigma_posterior(Ycrtl_complete, db_long_crtl, T_length, theta_comp_crtl, v0, S0)
    ...
    #estimate u
      ...
      #Bayes estimate
    u_trt=theta_comp_trt[T_length]
    u_crtl=theta_comp_crtl[T_length]
    u_diff = u_trt-u_crtl
    U_bayes[,k] = c(u_trt,u_crtl,u_diff)
  ...
  # Bayesian variance 
  var_bayes_est <- apply(U_bayes, 1, var)
  
  return(list(U = U,
              U_bayes = U_bayes,
              var_bayes_est  = var_bayes_est,
...
mc_sim <- function(N, nsim, burn_in_time, rep_time, B, method, missing_type){
  ...
  mean_trt_mcmc <- c()
  mean_diff_mcmc <- c()
  mean_ctl_bayes_mcmc <- c()
  mean_trt_bayes_mcmc <- c()
  mean_diff_bayes_mcmc <- c()
  var_bayes_est <- matrix(0, 3, nsim)
  ...
  for(s in 1:nsim){
    ...
    mean_mcmc_bayes <- res_mcmc$U_bayes
    ...
    mean_ctl_bayes_mcmc <- c(mean_ctl_bayes_mcmc, mean_mcmc_bayes[2,])
    mean_trt_bayes_mcmc <- c(mean_trt_bayes_mcmc, mean_mcmc_bayes[1,])
    mean_diff_bayes_mcmc <- c(mean_diff_bayes_mcmc, mean_mcmc_bayes[3,])
    var_bayes_est[,s] <- res_mcmc$var_bayes_est
  ...
  return(list(mean_ctl_mcmc = mean_ctl_mcmc,
              ...
              mean_ctl_bayes_mcmc = mean_ctl_bayes_mcmc,
              mean_trt_bayes_mcmc = mean_trt_bayes_mcmc,
              mean_diff_bayes_mcmc = mean_diff_bayes_mcmc,
              var_bayes_est = var_bayes_est,
              var_rubin_est = var_rubin_est,
...
Sigma <- matrix(c(rep(0.6, 25)), ncol=5)
for(i in 1:5){
  Sigma[i,i] =1
}
Pi <- c(80, rep(5, 4))/100
...
tmp1 <- mc_sim(100, nsim, burn_in_time, rep_time, B, "naive", "MCAR")

bc4_comp_eqm_100<-tmp1

save(bc4_comp_eqm_100, file = "bc4_comp_eqm_100.RData")
#save(tmp1, file = "Case3_T3_100_alt.RData")
#save(tmp1, file = "Case3_T5_100.RData")
#save(tmp1, file = "Case3_T5_100_alt.RData")
#save(tmp1, file = "Case3_T10_100.RData")
#save(tmp1, file = "Case3_T10_100_alt.RData")
#save(tmp1, file = "Case3_T20_100.RData")
#save(tmp1, file = "Case3_T20_100_alt.RData")
