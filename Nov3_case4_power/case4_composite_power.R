...
get_theta<-function(Y_matrix,Y_long,mu,Sigma,Omega){
  ...
  nj=ncol(Ycomp)
  Ybar=rowMeans(Ycomp)
  cov=ginv(ginv(Omega)+nj*ginv(Sigma))
  mean=cov%*%(ginv(Omega)%*%mu+nj*ginv(Sigma)%*%Ybar)
...
mcmc_sim <- function(db_comb, db_long, burn_in_time, rep_time){
  ...
  # print("post burn-in")
  # pb <- txtProgressBar(min = 0, max = rep_time, style = 3)
  for (k in 1:rep_time){
    #get the estimate of theta and Sigma
    theta_trt<-get_theta(db_comb_trt, db_long_trt,mu,Sigma_trt,Omega)
    theta_crtl<-get_theta(db_comb_crtl, db_long_crtl,mu,Sigma_crtl,Omega)
...
mc_sim <- function(N, nsim, burn_in_time, rep_time, B, method, missing_type){
  ...
  mean_mcmc <- mcmc_sim(db_comb, db_long, burn_in_time, rep_time)
  mean_ctl_mcmc <- mean_mcmc[2,]
  mean_trt_mcmc <- mean_mcmc[1,]
  mean_diff_mcmc <- mean_mcmc[3,]
  mle_mean_res[,1] <- mle_finder(db_comb, db_long, method = "naive")
  mle_sd_res[,1] <- sd_bootstrap(db_comb, db_long, B)
...
tmp1_p <- mc_sim(nn[1], nsim, burn_in_time, rep_time, B, "naive", "MCAR")

tmp2_p <- mc_sim(nn[2], nsim, burn_in_time, rep_time, B, "naive", "MCAR")

tmp3_p <- mc_sim(nn[3], nsim, burn_in_time, rep_time, B, "naive", "MCAR")

save(tmp1_p, tmp2_p, tmp3_p, file = "case4_composite_power.RData")
